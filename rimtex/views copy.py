...
from .models import UserProfile, Machine, MillMachine, MillInfo, MillShift, MillLayout, MillLine, SetupMachine, MachineType
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            permissions = {
                ...
                'machine_summ_view': request.POST.get('permissions_machine_summ_view') == 'on',
                'machine_summ_edit': request.POST.get('permissions_machine_summ_edit') == 'on',
            ...
            # Log the errors if the form is not valid
            for field in form:
...
def get_machine_models(request):
    ...
    machines = Machine.objects.filter(type=machine_type, status=True).values(
        'code', 'model', 'make_year', 'design', 'manufacturer', 'num_inputs', 'num_outputs', 'image', 'status'
    )
...
def millConfig(request):
    ...
    mill_machines = MillMachine.objects.all()
    machine_type = SetupMachine.objects.filter(is_add=True)
...
def millSetupMachine(request):
    machines = SetupMachine.objects.all()
...
def millInfo(request):
    mill, created = MillInfo.objects.get_or_create(id=1)
...
def millLayout(request):
    if request.method == 'POST':
        ...
        layout, created = MillLayout.objects.get_or_create(id=1)  
    ...
    layout = MillLayout.objects.first()  
...
def get_machines(request):
    machines = list(MillMachine.objects.all().values('id', 'machine_name', 'image', 'num_inputs', 'num_outputs'))
